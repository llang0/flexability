...
#flex app android version
FLEX_APP = "3.98.3.17.0"
...
class FlexUnlimited:
  ...
  allHeaders = {
    ...
    "FlexCapacityRequest": {
      ...
      "User-Agent": f"iOS/15.2 (iPhone Darwin) Model/iPhone Platform/iPhone13,3 RabbitiOS/{FLEX_APP}",
  ...
  def __init__(self, stop_event, queue = Queue) -> None:
    self.stop_event = stop_event
    self.rapidClick = False
    try:
      ...
      with open("config.json") as configFile:
        ...
        self.lastBlock = None # used for making sure missed blocks are only printed once
    ...
    if self.refreshToken == None:
      ...
      # set below variables to None, the reinit them after we get accessToken from Telegram
      self.__requestHeaders["x-amz-access-token"] = None
      self.__requestHeaders["X-Amz-Date"] = None
      self.serviceAreaIds = None
      self.__offersRequestBody = None
      Log.error("refreshToken not set -- Please register using Telegram")
    else:
  ...
  ### re inital all instance variables from new config file
  ### think about changing this - lots of duplicate code
  def re_init(self):
    try:
      with open("config.json") as configFile:
        config = json.load(configFile)
  ...
  # this is the function we will modify to work with telegram
  # telegram will call this function with the maplanding url as input
  def registerAccount(self, url):
    ...
    #print("Link: " + link)
    ## read maplanding url from text file
    #with open("authurl.txt", "r") as file:
    #  maplanding_url = file.read()
    #print(maplanding_url)
    ...
    ## here is the code for adding the new tokens to config
    try:
  ...
  def getAllServiceAreas(self)-> None:
    self.__requestHeaders["X-Amz-Date"] = FlexUnlimited.__getAmzDate()
    response = self.session.get(
    ...
    filename = "station_list.json"
    dict = {}
    for serviceArea in serviceAreaPoolList:
      #dict[serviceArea["serviceAreaName"]] = serviceArea["serviceAreaId"]
      dict[serviceArea["serviceAreaId"]] = serviceArea["serviceAreaName"]
  ...
  async def __acceptOffer(self, offer: Offer):
    ...
    # this looks like it makes you get a fresh token
    if request.status_code == 403:
      ...
      # test out offer.toString method
      print(offer.toString())
      for i in range(8):
        self.queue.put(f"💸💸💸💸💸💸💸💸\n Successfully accepted an offer!!!\n 💸💸💸💸💸💸💸💸\n")
        await asyncio.sleep(0.5)
        
      self.queue.put(f"Successfully accepted an offer.\n" + offer.toString())
      self.stop_event.set()
      self.queue.put("Scanning stopped")
      #Log.email_alert(offer.toString())
      #self.rapidcount = 1
      # Log.block -> print out caught block message
      # 
    # elif request.status_code == 307:
    #   Log.info(f"CAPTCHA detected")
    #   url = request.json()['challengeMetadata']['WebUrl']
    #   print(f"line 461:\n{request.json}")
    #   self.queue.put("❗️ CAPTCHA detected \n ❗️ please /stopscan")
    #   self.queue.put(f"❗️ Click on the following url to solve the CAPTCHA: \n{url}")
    #   self.stop_event.set()
    else:
      ...
      print(offer.toString())
      self.rapidcount = 1
      if not offer.id == self.lastBlock:
        self.queue.put(f"Unable to accept an offer. Request returned status code {request.status_code}\n" + offer.toString())
        self.lastBlock = offer.id 
      self.stop_event.set()
      self.queue.put("Scanning stopped")
      # Log.block -> print out missed block message
      # 410 is misseg block code
  ...
  async def run(self):
    ...
    #check for station_filters.json and generate if it doesn't exits
    #do this first so offer class can work
    if not os.path.exists("station_list.json"):
      self.getAllServiceAreas()

    Log.info("Starting job search...")
    #while self.__retryCount < self.retryLimit:

    # run main loop as long as stop event isn't set
    while not self.stop_event.is_set():
      offersResponse = self.__getOffers()
      # if successful request
      if offersResponse.status_code == 200:
        # get list of offers from dict
        ...
        for offer in currentOffers:
          # init custom offer class for each offer
          offerResponseObject = Offer(offerResponseObject=offer)
          # check against filters then accept offer
          # offers that go in here are instances of offer class
        ...
        self.queue.put("Rate limit reached. Waiting for " + str(minutes_to_wait) + " minutes.")
        await asyncio.sleep(minutes_to_wait * 60)
        Log.info("Resuming search.")
        self.queue.put("Resuming search.")
      else:
        Log.error(offersResponse.json())
        break

      # test out message queue
      #Log.add_message("testing :)")
      #self.queue.put("testing :)")
      if self.rapidClick:
        if self.rapidcount < 5:
          self.rapidcount += 1
          await asyncio.sleep(random.uniform(0.1, 0.2))
        else:
          value = 1
          while value < 1.6: #1.6 is our anecdotal minimum refresh interval
            value = random.gauss(self.refreshInterval, 0.5)
