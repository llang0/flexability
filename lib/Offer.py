...
class Offer:
    ...
    def __init__(self, offerResponseObject: object) -> None:
        ...
        self.location = self.__getStationName(offerResponseObject.get('serviceAreaId'))
        ...
        #self.stationName = self.__getStationName(self.location)

    def __getStationName(self, serviceAreaId):
        filename = "station_list.json"
        if os.path.exists(filename):
            with open(filename, 'r') as f:
                data = json.load(f)
                stationName = data[serviceAreaId]
        else: 
            Log.error("please make sure a properly formatted station_list.json exists")
        return stationName
